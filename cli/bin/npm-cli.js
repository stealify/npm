...
if !(require('node:fs').existsSync(__dirname+'/../npm-src')) {
  // Using system installed npm if possible
  //require('node:child_process').exec(__dirname+'/node_modules', { stdio: "inhire"},
  //(error, _stdout, _stderr) => (error) && console.error(`exec error: ${error}`));
}

require('../node_modules/npm-src/lib/cli.js')(process)
